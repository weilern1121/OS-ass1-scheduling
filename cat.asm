...
main(int argc, char *argv[])
{
  ...
  5d:	c7 44 24 04 73 08 00 	movl   $0x873,0x4(%esp)
  64:	00 
...
cat(int fd)
{
  ...
  d2:	b9 80 0b 00 00       	mov    $0xb80,%ecx
...
putc(int fd, char c)
{
 ...
 4c7:	e8 cc fe ff ff       	call   398 <write>
    buf[i++] = digits[x % base];
  }while((x /= base) != 0);
  if(neg)
    buf[i++] = '-';
...
putc(int fd, char c)
{
 ...
 53b:	e8 58 fe ff ff       	call   398 <write>
...
putc(int fd, char c)
{
 ...
 5a6:	e8 ed fd ff ff       	call   398 <write>
 ...
 5c1:	e8 d2 fd ff ff       	call   398 <write>
  int c, i, state;
  uint *ap;
...
putc(int fd, char c)
{
 ...
 647:	e8 4c fd ff ff       	call   398 <write>
      } else if(c == 's'){
        s = (char*)*ap;
        ap++;
        if(s == 0)
...
putc(int fd, char c)
{
 ...
 676:	e8 1d fd ff ff       	call   398 <write>
      } else {
        // Unknown % sequence.  Print it to draw attention.
        putc(fd, '%');
        putc(fd, c);
...
malloc(uint nbytes)
{
 ...
 779:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 77c:	8b 15 60 0b 00 00    	mov    0xb60,%edx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
 ...
 7c7:	76 37                	jbe    800 <malloc+0x90>
    ...
    if(p == freep)
 7c9:	39 05 60 0b 00 00    	cmp    %eax,0xb60
 ...
 7d6:	e8 25 fc ff ff       	call   400 <sbrk>
  if(p == (char*)-1)
 ...
 7e9:	e8 e2 fe ff ff       	call   6d0 <free>
  return freep;
 7ee:	8b 15 60 0b 00 00    	mov    0xb60,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
 ...
 824:	eb e9                	jmp    80f <malloc+0x9f>
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 826:	b8 64 0b 00 00       	mov    $0xb64,%eax
 82b:	ba 64 0b 00 00       	mov    $0xb64,%edx
    base.s.size = 0;
 830:	31 c9                	xor    %ecx,%ecx
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 832:	a3 60 0b 00 00       	mov    %eax,0xb60
    base.s.size = 0;
 837:	b8 64 0b 00 00       	mov    $0xb64,%eax
  Header *p, *prevp;
  uint nunits;
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 83c:	89 15 64 0b 00 00    	mov    %edx,0xb64
    base.s.size = 0;
 842:	89 0d 68 0b 00 00    	mov    %ecx,0xb68
