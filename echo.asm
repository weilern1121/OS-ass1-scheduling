...
main(int argc, char *argv[])
{
  ...
  1c:	eb 28                	jmp    46 <main+0x46>
  1e:	66 90                	xchg   %ax,%ax
    printf(1, "%s%s", argv[i], i+1 < argc ? " " : "\n");
  20:	b8 80 07 00 00       	mov    $0x780,%eax
  25:	ba 82 07 00 00       	mov    $0x782,%edx
  2a:	89 44 24 0c          	mov    %eax,0xc(%esp)
  2e:	8b 44 9f fc          	mov    -0x4(%edi,%ebx,4),%eax
  ...
  46:	43                   	inc    %ebx
  47:	39 de                	cmp    %ebx,%esi
  49:	75 d5                	jne    20 <main+0x20>
  4b:	c7 44 24 0c 87 07 00 	movl   $0x787,0xc(%esp)
  52:	00 
  53:	8b 44 b7 fc          	mov    -0x4(%edi,%esi,4),%eax
  57:	c7 44 24 04 82 07 00 	movl   $0x782,0x4(%esp)
  5e:	00 
  ...
  76:	e8 3d 02 00 00       	call   2b8 <exit>
  7b:	66 90                	xchg   %ax,%ax
  7d:	66 90                	xchg   %ax,%ax
  7f:	90                   	nop
...
putc(int fd, char c)
{
 ...
 548:	89 45 d0             	mov    %eax,-0x30(%ebp)
        if(s == 0)
          s = "(null)";
 54b:	b8 89 07 00 00       	mov    $0x789,%eax
...
free(void *ap)
{
 ...
 67c:	8b 4b fc             	mov    -0x4(%ebx),%ecx
    ...
    p->s.ptr = bp;
  freep = p;
 67f:	a3 20 0a 00 00       	mov    %eax,0xa20
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
 ...
 6a9:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 6ac:	8b 15 20 0a 00 00    	mov    0xa20,%edx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
 ...
 6f7:	76 37                	jbe    730 <malloc+0x90>
    ...
    if(p == freep)
 6f9:	39 05 20 0a 00 00    	cmp    %eax,0xa20
 ...
 719:	e8 e2 fe ff ff       	call   600 <free>
  return freep;
 71e:	8b 15 20 0a 00 00    	mov    0xa20,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
 ...
 73c:	89 78 04             	mov    %edi,0x4(%eax)
      ...
      freep = prevp;
 73f:	89 15 20 0a 00 00    	mov    %edx,0xa20
      return (void*)(p + 1);
 ...
 754:	eb e9                	jmp    73f <malloc+0x9f>
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 756:	b8 24 0a 00 00       	mov    $0xa24,%eax
 75b:	ba 24 0a 00 00       	mov    $0xa24,%edx
    base.s.size = 0;
 760:	31 c9                	xor    %ecx,%ecx
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 762:	a3 20 0a 00 00       	mov    %eax,0xa20
    base.s.size = 0;
 767:	b8 24 0a 00 00       	mov    $0xa24,%eax
  Header *p, *prevp;
  uint nunits;
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 76c:	89 15 24 0a 00 00    	mov    %edx,0xa24
    base.s.size = 0;
 772:	89 0d 28 0a 00 00    	mov    %ecx,0xa28
